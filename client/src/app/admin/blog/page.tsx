...
const emptyForm = {
  title: '', content: '', excerpt: '', category: '', author: 'BK Engineering', published: 'false',
};
...
export default function AdminBlogPage() {
  ...
  const openEdit = (p: BlogPost) => {
    ...
    setForm({
      ...
      category: p.category || '', author: p.author, published: p.published ? 'true' : 'false',
    });
    setThumbnailFile(null);
    setShowModal(true);
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editing) {
        await adminAPI.updateBlogPost(editing.id, fd);
  ...
  return (
    <AdminLayout>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-black text-primary font-heading">Blog</h1>
          <p className="text-gray-500 text-sm mt-1">{posts.length} article(s)</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
              ...
              {posts.map(post => (
                <tr key={post.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-6 py-4">
                    <div className="font-medium text-primary text-sm line-clamp-1">{post.title}</div>
                    <div className="text-xs text-gray-400 mt-0.5">par {post.author}</div>
                  </td>
                  ...
                  <td className="px-6 py-4 hidden lg:table-cell">
                    <span className={`flex items-center gap-1 text-xs font-medium ${post.published ? 'text-green-600' : 'text-gray-400'}`}>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center justify-end gap-2">
                      <button onClick={() => openEdit(post)} className="p-2 hover:bg-blue-50 rounded-lg text-blue-600 transition-colors">
                        <Pencil className="w-4 h-4" />
                      </button>
                      <button onClick={() => handleDelete(post.id)} className="p-2 hover:bg-red-50 rounded-lg text-red-500 transition-colors">
          ...
          {posts.length === 0 && (
            <div className="text-center py-12 text-gray-400">Aucun article. Créez-en un !</div>
          )}
        </div>
      )}
          ...
          <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl">
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4">
              ...
              <div>
                ...
                <textarea rows={2} value={form.excerpt} onChange={e => setForm(p => ({ ...p, excerpt: e.target.value }))} className="input-field resize-none" placeholder="Résumé court de l'article..." />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Contenu *</label>
                <textarea required rows={10} value={form.content} onChange={e => setForm(p => ({ ...p, content: e.target.value }))} className="input-field resize-none font-mono text-sm" placeholder="Contenu HTML de l'article..." />
                <p className="text-xs text-gray-400 mt-1">Vous pouvez utiliser du HTML pour la mise en forme.</p>
              </div>
              <div>
              ...
              <div className="flex items-center gap-3">
                <input
                  ...
                  onChange={e => setForm(p => ({ ...p, published: e.target.checked ? 'true' : 'false' }))}
